  ...
  return (
    ...
    >
      <div className=" w-full h-full  bg-white z-101 flex flex-col  items-center border-2 border-red-800">
        ...
        <div className="flex  justify-center my-2 border-2 border-red-700 w-full h-4/5 m-0 p-0 ">
          <ButtonLight height={height} text=" ‹" onClick={prevHandler} />
          <div className="flex w-1/2 border-0 border-sky-400 overflow-hidden">
            {newimages?.map((image, index) => (
              <div
                key={index}
                className="flex-shrink-0 p-1 m-0 border-0 border-red-400 transition-transform duration-300 ease-in-out overflow-hidden w-full"
                ref={imageRef}
                style={{ transform: `translateX(-${0}px)` }}
              >
                {/* NEM TUDTAM BEÁLLÍTANI A KÉP MAGASSÁGÁT MEGFELELŐRE */}
                <div className="border-4 flex flex-col border-lime-400 items-center  overflow-hidden">
                  <img
                    src={image.image}
                    alt={altText}
                    className="border-4 "
                  ...
                  />
                  <div className="text-center w-1/2">{image.name}</div>
                </div>
              </div>
            ))}
          </div>
